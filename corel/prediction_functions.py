...
def get_cosine_predictions(hidden_reps, output_weights, gamma=None, device=None):
    assert (hidden_reps.shape[1] == output_weights.shape[0])
...
def get_gaussian_predictions(hidden_reps, output_weights, gamma=0.5, device=None):
    assert(hidden_reps.shape[1] == output_weights.shape[0])
    ...
    # getting the first parts of the equation
